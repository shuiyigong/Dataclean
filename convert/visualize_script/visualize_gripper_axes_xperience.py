from __future__ import annotations

import argparse
import json
from pathlib import Path

import cv2
import numpy as np


DEFAULT_NPZ = Path(
    "/mnt/project_rlinf/runze/ml-egodex/convert/output/xperience_npz/"
    "00986a68-7248-492a-befc-daae0c3f9866__ep1.npz"
)
DEFAULT_MANIFEST = Path("/mnt/project_rlinf/runze/ml-egodex/convert/output/xperience_npz/manifest.json")
DEFAULT_OUTPUT = Path(
    "/mnt/project_rlinf/runze/ml-egodex/convert/output/visualize/xperience/"
    "00986a68-7248-492a-befc-daae0c3f9866__ep1.mp4"
)

AXIS_COLORS = {
    "x": (255, 40, 40),   # RGB red
    "y": (40, 220, 40),   # RGB green
    "z": (40, 120, 255),  # RGB blue
}
BAR_BG = (20, 20, 20)
BAR_BORDER = (255, 255, 255)
LEFT_BAR_COLOR = (255, 190, 40)
RIGHT_BAR_COLOR = (40, 220, 255)


def gripper_pose_to_current_camera(
    position: np.ndarray,
    rotation: np.ndarray,
    camera_extrinsics_world: np.ndarray,
    frame_idx: int,
) -> tuple[np.ndarray, np.ndarray]:
    """Transform a gripper pose from the first camera frame to frame_idx camera coordinates."""

    first_camera_to_world = camera_extrinsics_world[0]
    world_to_current_camera = np.linalg.inv(camera_extrinsics_world[frame_idx])
    first_camera_to_current_camera = world_to_current_camera @ first_camera_to_world
    R = first_camera_to_current_camera[:3, :3].astype(np.float32)
    t = first_camera_to_current_camera[:3, 3].astype(np.float32)
    return (R @ position + t).astype(np.float32), (R @ rotation).astype(np.float32)


def points_first_camera_to_current_camera(
    points: np.ndarray,
    camera_extrinsics_world: np.ndarray,
    frame_idx: int,
) -> np.ndarray:
    first_camera_to_world = camera_extrinsics_world[0]
    world_to_current_camera = np.linalg.inv(camera_extrinsics_world[frame_idx])
    first_camera_to_current_camera = world_to_current_camera @ first_camera_to_world
    points_h = np.concatenate(
        [points.astype(np.float64), np.ones((points.shape[0], 1), dtype=np.float64)],
        axis=-1,
    )
    return (first_camera_to_current_camera @ points_h.T).T[:, :3].astype(np.float32)


def project_points(points_cam: np.ndarray, intrinsic: np.ndarray) -> np.ndarray:
    intrinsic = intrinsic.reshape(3, 3)
    if not np.isfinite(points_cam).all() or not np.isfinite(intrinsic).all():
        return np.full((len(points_cam), 2), np.nan, dtype=np.float32)
    points_2d, _ = cv2.projectPoints(
        points_cam.astype(np.float32),
        np.eye(3, dtype=np.float32),
        np.zeros(3, dtype=np.float32),
        intrinsic.astype(np.float32),
        distCoeffs=np.zeros(5, dtype=np.float32),
    )
    return points_2d.reshape(-1, 2)


def camera_matrix(intrinsics: np.ndarray) -> np.ndarray:
    intrinsics = np.asarray(intrinsics, dtype=np.float32)
    if intrinsics.shape == (3, 3):
        return intrinsics.astype(np.float32)
    flat = intrinsics.reshape(-1)
    if flat.size == 9 and flat[6] == 0.0 and flat[7] == 0.0 and flat[8] == 1.0:
        return flat.reshape(3, 3).astype(np.float32)
    if flat.size >= 4:
        fx, fy, cx, cy = flat[:4]
        return np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]], dtype=np.float32)
    raise ValueError(f"Unsupported camera_intrinsics shape: {intrinsics.shape}")


def scale_camera_matrix(intrinsic: np.ndarray, intrinsics_raw: np.ndarray, width: int, height: int) -> np.ndarray:
    raw = np.asarray(intrinsics_raw, dtype=np.float32).reshape(-1)
    if raw.size >= 6 and raw[4] > 0 and raw[5] > 0:
        sx = float(width) / float(raw[4])
        sy = float(height) / float(raw[5])
        scaled = intrinsic.copy()
        scaled[0, 0] *= sx
        scaled[0, 2] *= sx
        scaled[1, 1] *= sy
        scaled[1, 2] *= sy
        return scaled.astype(np.float32)
    return intrinsic.astype(np.float32)


def rotation_6d_to_matrix(rot6d: np.ndarray) -> np.ndarray:
    cols = rot6d.reshape(-1, 2, 3)
    a1 = cols[:, 0]
    a2 = cols[:, 1]
    b1 = a1 / np.maximum(np.linalg.norm(a1, axis=-1, keepdims=True), 1e-8)
    a2_orth = a2 - np.sum(b1 * a2, axis=-1, keepdims=True) * b1
    b2 = a2_orth / np.maximum(np.linalg.norm(a2_orth, axis=-1, keepdims=True), 1e-8)
    b3 = np.cross(b1, b2)
    return np.stack([b1, b2, b3], axis=-1).astype(np.float32)


def gripper_tracks(data: np.lib.npyio.NpzFile, action_key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if all(key in data.files for key in ["left_position", "right_position", "left_rotation", "right_rotation"]):
        return data["left_position"], data["right_position"], data["left_rotation"], data["right_rotation"]

    if action_key not in data.files:
        raise KeyError(
            f"{action_key!r} not found and left/right position/rotation fields are missing. "
            f"Available keys: {data.files}"
        )
    action = data[action_key].astype(np.float32)
    if action.shape[1] != 20:
        raise ValueError(f"Expected bimanual 20D action, got {action.shape}")
    left = action[:, :10]
    right = action[:, 10:20]
    return (
        left[:, :3],
        right[:, :3],
        rotation_6d_to_matrix(left[:, 3:9]),
        rotation_6d_to_matrix(right[:, 3:9]),
    )


def validity_array(data: np.lib.npyio.NpzFile, hand: str, n: int) -> np.ndarray:
    key = f"{hand}_keypoints_valid"
    if key in data.files:
        return data[key].astype(bool)
    if "keep_mask_pre_filter" in data.files:
        return data["keep_mask_pre_filter"].astype(bool)
    return np.ones(n, dtype=bool)


def draw_arrow(
    image_rgb: np.ndarray,
    start_cam: np.ndarray,
    end_cam: np.ndarray,
    intrinsic: np.ndarray,
    color: tuple[int, int, int],
    thickness: int,
) -> None:
    if not np.isfinite(start_cam).all() or not np.isfinite(end_cam).all():
        return
    if start_cam[2] <= 1e-4 or end_cam[2] <= 1e-4:
        return
    p0, p1 = project_points(np.stack([start_cam, end_cam]), intrinsic)
    if not np.isfinite(p0).all() or not np.isfinite(p1).all():
        return
    h, w = image_rgb.shape[:2]
    margin = 100
    if (
        max(p0[0], p1[0]) < -margin
        or min(p0[0], p1[0]) > w + margin
        or max(p0[1], p1[1]) < -margin
        or min(p0[1], p1[1]) > h + margin
    ):
        return
    p0_i = tuple(np.round(p0).astype(int))
    p1_i = tuple(np.round(p1).astype(int))
    cv2.arrowedLine(image_rgb, p0_i, p1_i, color=color, thickness=thickness, tipLength=0.25)


def draw_frame_axes(
    image_rgb: np.ndarray,
    position: np.ndarray,
    rotation: np.ndarray,
    intrinsic: np.ndarray,
    *,
    axis_length: float,
    thickness: int,
    label: str | None = None,
) -> None:
    if not np.isfinite(position).all() or not np.isfinite(rotation).all():
        return
    if position[2] <= 1e-4:
        return
    for axis_idx, axis_name in enumerate(["x", "y", "z"]):
        end = position + axis_length * rotation[:, axis_idx]
        draw_arrow(image_rgb, position, end, intrinsic, AXIS_COLORS[axis_name], thickness)

    center_2d = project_points(position[None], intrinsic)[0]
    if not np.isfinite(center_2d).all():
        return
    cv2.circle(image_rgb, tuple(np.round(center_2d).astype(int)), max(3, thickness + 1), (255, 255, 255), -1)
    if label:
        xy = tuple(np.round(center_2d + np.array([8, -8])).astype(int))
        cv2.putText(image_rgb, label, xy, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)


def draw_hand_points(
    image_rgb: np.ndarray,
    points_cam: np.ndarray,
    intrinsic: np.ndarray,
    color: tuple[int, int, int],
    *,
    radius: int,
) -> None:
    if not np.isfinite(points_cam).all():
        return
    visible = points_cam[:, 2] > 1e-4
    if not bool(visible.any()):
        return
    pts_2d = project_points(points_cam[visible], intrinsic)
    h, w = image_rgb.shape[:2]
    for p in pts_2d:
        if not np.isfinite(p).all():
            continue
        x, y = np.round(p).astype(int)
        if -20 <= x <= w + 20 and -20 <= y <= h + 20:
            cv2.circle(image_rgb, (x, y), radius, color, -1)


def draw_confidence_bar(
    image_rgb: np.ndarray,
    value: float,
    *,
    side: str,
    color: tuple[int, int, int],
    label: str,
) -> None:
    h, w = image_rgb.shape[:2]
    bar_height = int(h * 0.38)
    bar_width = max(18, int(w * 0.012))
    top = int(h * 0.12)
    bottom = top + bar_height
    x0 = int(w * 0.035) if side == "left" else int(w * 0.965) - bar_width
    x1 = x0 + bar_width

    value = float(np.clip(value, 0.0, 1.0))
    fill_top = bottom - int(round(value * bar_height))

    overlay = image_rgb.copy()
    cv2.rectangle(overlay, (x0, top), (x1, bottom), BAR_BG, -1)
    cv2.rectangle(overlay, (x0, fill_top), (x1, bottom), color, -1)
    cv2.rectangle(overlay, (x0, top), (x1, bottom), BAR_BORDER, 2)
    cv2.addWeighted(overlay, 0.65, image_rgb, 0.35, 0, dst=image_rgb)

    text_x = x0 - 8 if side == "left" else x0 - 20
    cv2.putText(image_rgb, label, (text_x, max(24, top - 14)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, BAR_BORDER, 2, cv2.LINE_AA)
    cv2.putText(image_rgb, f"{value:.2f}", (x0 - 14, bottom + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, BAR_BORDER, 2, cv2.LINE_AA)


def draw_axis_legend(image_rgb: np.ndarray, *, labels: bool = True) -> None:
    h, w = image_rgb.shape[:2]
    origin = np.array([w - 120, h - 56], dtype=np.int32)
    axes = [
        ("x", np.array([42, 0], dtype=np.int32)),
        ("y", np.array([0, -42], dtype=np.int32)),
        ("z", np.array([30, 30], dtype=np.int32)),
    ]
    overlay = image_rgb.copy()
    cv2.rectangle(overlay, tuple(origin + [-20, -58]), tuple(origin + [78, 46]), (20, 20, 20), -1)
    cv2.addWeighted(overlay, 0.45, image_rgb, 0.55, 0, dst=image_rgb)
    cv2.circle(image_rgb, tuple(origin), 4, (255, 255, 255), -1)
    for name, delta in axes:
        end = origin + delta
        color = AXIS_COLORS[name]
        cv2.arrowedLine(image_rgb, tuple(origin), tuple(end), color=color, thickness=3, tipLength=0.25)
        if labels:
            cv2.putText(image_rgb, name, tuple(end + np.array([4, 4], dtype=np.int32)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)


def draw_status_text(image_rgb: np.ndarray, frame_idx: int, frame_name: str, valid: bool) -> None:
    state = "valid" if valid else "invalid"
    text = f"frame {frame_idx:04d} | {state} | axes: x red, y green, z blue | gripper frame: {frame_name}"
    color = (255, 255, 255) if valid else (255, 80, 80)
    cv2.putText(image_rgb, text, (16, image_rgb.shape[0] - 18), cv2.FONT_HERSHEY_SIMPLEX, 0.48, color, 2, cv2.LINE_AA)


def infer_mp4_path(npz_path: Path, manifest_path: Path | None) -> Path:
    if manifest_path and manifest_path.exists():
        manifest = json.loads(manifest_path.read_text())
        for ep in manifest.get("episodes", []):
            ep_npz_path = Path(ep.get("npz_path", ""))
            if ep_npz_path == npz_path or ep_npz_path.name == npz_path.name or ep.get("episode_id") == npz_path.stem:
                mp4_path = ep.get("video_path") or ep.get("mp4_path")
                if mp4_path:
                    return Path(mp4_path)
    sibling = npz_path.with_suffix(".mp4")
    if sibling.exists():
        return sibling
    raise ValueError(f"Could not infer mp4 path for {npz_path}; pass --mp4 or --manifest")


def visualize(args: argparse.Namespace) -> None:
    npz_path = args.npz.resolve()
    manifest_path = args.manifest.resolve() if args.manifest else None
    mp4_path = args.mp4.resolve() if args.mp4 else infer_mp4_path(npz_path, manifest_path)
    output_path = args.output.resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)

    data = np.load(npz_path)
    intrinsics_raw = data["camera_intrinsics"]
    intrinsics = camera_matrix(intrinsics_raw)
    left_position, right_position, left_rotation, right_rotation = gripper_tracks(data, args.action_key)
    n = min(len(left_position), args.max_frames if args.max_frames else len(left_position))

    left_valid = validity_array(data, "left", len(left_position))
    right_valid = validity_array(data, "right", len(left_position))
    keep_valid = data["keep_mask_pre_filter"].astype(bool) if "keep_mask_pre_filter" in data.files else (left_valid & right_valid)
    camera_extrinsics_world = data["camera_extrinsics_world"] if "camera_extrinsics_world" in data.files else None
    if args.gripper_frame == "first-camera":
        if camera_extrinsics_world is None:
            raise KeyError("camera_extrinsics_world is required for --gripper-frame first-camera")
        if len(camera_extrinsics_world) < n:
            raise ValueError(f"camera_extrinsics_world has {len(camera_extrinsics_world)} frames, but visualization needs {n}")

    hand_points_frame = "current-camera"
    if args.gripper_frame == "first-camera" and all(
        key in data.files for key in ["left_hand_21_first_camera", "right_hand_21_first_camera"]
    ):
        left_hand_21 = data["left_hand_21_first_camera"]
        right_hand_21 = data["right_hand_21_first_camera"]
        hand_points_frame = "first-camera"
    else:
        left_hand_21 = data["left_hand_21_cam"] if "left_hand_21_cam" in data.files else None
        right_hand_21 = data["right_hand_21_cam"] if "right_hand_21_cam" in data.files else None

    cap = cv2.VideoCapture(str(mp4_path))
    if not cap.isOpened():
        raise FileNotFoundError(f"Could not open video: {mp4_path}")

    fps = cap.get(cv2.CAP_PROP_FPS) or 20
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    intrinsics = scale_camera_matrix(intrinsics, intrinsics_raw, width, height)
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height))

    frame_idx = 0
    while frame_idx < n:
        ok, frame_bgr = cap.read()
        if not ok:
            break
        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)

        if args.gripper_frame == "first-camera":
            assert camera_extrinsics_world is not None
            left_pos, left_rot = gripper_pose_to_current_camera(left_position[frame_idx], left_rotation[frame_idx], camera_extrinsics_world, frame_idx)
            right_pos, right_rot = gripper_pose_to_current_camera(right_position[frame_idx], right_rotation[frame_idx], camera_extrinsics_world, frame_idx)
        else:
            left_pos, left_rot = left_position[frame_idx], left_rotation[frame_idx]
            right_pos, right_rot = right_position[frame_idx], right_rotation[frame_idx]

        if args.draw_hand_points and left_hand_21 is not None and frame_idx < len(left_hand_21):
            left_points = left_hand_21[frame_idx]
            if hand_points_frame == "first-camera":
                assert camera_extrinsics_world is not None
                left_points = points_first_camera_to_current_camera(left_points, camera_extrinsics_world, frame_idx)
            draw_hand_points(frame_rgb, left_points, intrinsics, LEFT_BAR_COLOR, radius=args.point_radius)
        if args.draw_hand_points and right_hand_21 is not None and frame_idx < len(right_hand_21):
            right_points = right_hand_21[frame_idx]
            if hand_points_frame == "first-camera":
                assert camera_extrinsics_world is not None
                right_points = points_first_camera_to_current_camera(right_points, camera_extrinsics_world, frame_idx)
            draw_hand_points(frame_rgb, right_points, intrinsics, RIGHT_BAR_COLOR, radius=args.point_radius)

        if left_valid[frame_idx]:
            draw_frame_axes(
                frame_rgb,
                left_pos,
                left_rot,
                intrinsics,
                axis_length=args.axis_length,
                thickness=args.thickness,
                label="L" if args.labels else None,
            )
        if right_valid[frame_idx]:
            draw_frame_axes(
                frame_rgb,
                right_pos,
                right_rot,
                intrinsics,
                axis_length=args.axis_length,
                thickness=args.thickness,
                label="R" if args.labels else None,
            )

        if not args.no_confidence_bars:
            draw_confidence_bar(frame_rgb, float(left_valid[frame_idx]), side="left", color=LEFT_BAR_COLOR, label="L valid")
            draw_confidence_bar(frame_rgb, float(right_valid[frame_idx]), side="right", color=RIGHT_BAR_COLOR, label="R valid")
        if not args.no_legend:
            draw_axis_legend(frame_rgb)
            draw_status_text(frame_rgb, frame_idx, args.gripper_frame, bool(keep_valid[frame_idx]))

        writer.write(cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR))
        frame_idx += 1

    cap.release()
    writer.release()
    print(f"Wrote {frame_idx} frames to {output_path}")
    print(f"Source npz: {npz_path}")
    print(f"Source mp4: {mp4_path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Draw converted Xperience gripper coordinate axes on stereo_left video.")
    parser.add_argument("--npz", type=Path, default=DEFAULT_NPZ, help="Converted Xperience episode .npz.")
    parser.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST, help="Manifest used to infer the source video.")
    parser.add_argument("--mp4", type=Path, default=None, help="Source video. If omitted, inferred from manifest.")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--axis-length", type=float, default=0.06, help="Axis arrow length in meters.")
    parser.add_argument("--thickness", type=int, default=5)
    parser.add_argument("--max-frames", type=int, default=None)
    parser.add_argument("--labels", action="store_true", help="Draw L/R labels at axis origins.")
    parser.add_argument("--draw-hand-points", action="store_true", help="Overlay all 21 hand keypoints in addition to gripper axes.")
    parser.add_argument("--point-radius", type=int, default=3)
    parser.add_argument("--no-confidence-bars", action="store_true", help="Disable dynamic hand validity bars.")
    parser.add_argument("--no-legend", action="store_true", help="Disable axis legend and frame text.")
    parser.add_argument("--action-key", default="action_smooth", help="Action array used when pose fields are absent.")
    parser.add_argument(
        "--gripper-frame",
        choices=["first-camera", "current-camera"],
        default="first-camera",
        help=(
            "Coordinate frame of left/right gripper poses in the npz. New Xperience step1 outputs use "
            "first-camera; old outputs used current-camera."
        ),
    )
    return parser.parse_args()


if __name__ == "__main__":
    visualize(parse_args())
